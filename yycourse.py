...
def prev_check(_drivr,cinfo): # returns [,[list(starts from 1)]]
    ...
    for i in range(1,num+1): # 逐个看学习状况中的每个预习完成情况（序号从1开始）
        ...
        while not selenium_wait(_drivr, spot):
            if myerr < 2:
                detail(_drivr, cinfo)
                Clickx(_drivr, COURSE_TABS_STAT)
            else:
                break
            myerr += 1
...
def prev(_drivr,cinfo): # 预习
    ...
    while not check_res[0]: # 没有学完
        ...
        myerr = 0
        while not selenium_wait(_drivr, COURSE_TABS_PREV):
            if myerr < 2:
                detail(_drivr, cinfo)
            else:
                break
        ...
        for i in check_res[1]: # 遍历每个没完成的序号
            ...
            while not selenium_wait(_drivr, spot):
                if myerr < 2:
                    detail(_drivr, cinfo)
                    Clickx(_drivr, COURSE_TABS_PREV)
                else:
                    break
                myerr += 1
...
def test(_drivr,cinfo): # 自测
    ...
    while not check_res[0]: # 如果自测分数检查过后发现不是100
        ...
        myerr = 0
        while not selenium_wait(_drivr, COURSE_TABS_TEST):
            if myerr < 2:
                detail(_drivr, cinfo)
            else:
                break
        ...
        for i in check_res[1]: # 遍历没有做满分的自测题序号（从1开始）
            ...
            while not selenium_wait(_drivr, spot):
                if myerr < 2:
                    detail(_drivr, cinfo)
                    Clickx(_drivr, COURSE_TABS_TEST)
                else:
                    break
                myerr += 1
...
def perform_exam(_drivr,cinfo,cur,user_id): # 进行仿真考核 # [id,simp_nm,p,t,e,credit_hr]
    ...
    while not selenium_wait(_drivr, spot):
        if myerr < 2:
            detail(_drivr, cinfo)
            Clickx(_drivr, COURSE_TABS_EXAM)
        else:
            break
        myerr += 1
...
def exam(_drivr,cinfo,user_id): # 仿真考核
    ...
    while not check_res[0]: # 没有完成
        ...
        myerr = 0
        while not selenium_wait(_drivr, COURSE_TABS_EXAM):
            if myerr < 2:
                detail(_drivr, cinfo)
            else:
                break
